...
publishes prebuilt `linux/amd64` binaries.

The [CI workflow](.github/workflows/ci.yml) downloads those binaries, runs them
to regenerate the rule-sets, and commits the two folders back to `master`. It
runs on push, on manual dispatch, and daily at 01:00 UTC. The build runs on the
CI runner because the binaries are `linux/amd64`.
